...
class ConfigsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create config" do
    assert_difference('Config.count') do
      post configs_url, params: { config: { actv_sts: @config.actv_sts, asset_id: @config.asset_id, cust_id: @config.cust_id, loan_nbr: @config.loan_nbr } }
    end
  ...
  test "should update config" do
    patch config_url(@config), params: { config: { actv_sts: @config.actv_sts, asset_id: @config.asset_id, cust_id: @config.cust_id, loan_nbr: @config.loan_nbr } }
